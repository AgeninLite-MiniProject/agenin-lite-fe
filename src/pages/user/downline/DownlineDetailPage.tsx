...
// Mock data (since API is not yet connected)
const mockDetail = {
  id: 1,
  name: "Budi Santoso",
  phone: "+628123456789",
  ...
  status: "ACTIVE",
  joined_at: "06 Sep 2026",
  last_transaction_at: "10 Okt 2026",
  profit_income_from_agent: 40000,
...
const mockTransactions = [
  {
    ...
    amount: 500000,
    status: "COMPLETED",
    completed_at: "10 Okt 2026, 14:00",
    commission_earned: 5000
  },
  {
    id: 2,
    product_name: "Indomie Goreng 2 Dus",
    quantity: 2,
    amount: 220000,
    status: "COMPLETED",
    completed_at: "08 Okt 2026, 09:30",
        ...
        <CardContent className="p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start">
            <Avatar className="h-24 w-24 md:h-28 md:w-28 border-4 border-slate-50 shadow-sm bg-slate-100 shrink-0">
              <AvatarImage src={mockDetail.image} alt={mockDetail.name} />
              <AvatarFallback className="text-3xl font-bold">{mockDetail.name.charAt(0)}</AvatarFallback>
            </Avatar>
            
            <div className="flex-1 space-y-6 w-full">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="text-2xl font-bold text-slate-900">{mockDetail.name}</h1>
                  <Badge variant="secondary" className="bg-green-100/70 text-green-700 hover:bg-green-100/70 border-transparent text-[10px] px-2.5 py-0.5 rounded-md font-medium">
                    {mockDetail.status}
                  </Badge>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                <div className="bg-[#F8F9FE] p-4 rounded-2xl border border-transparent hover:border-slate-100 transition-colors">
                  ...
                  <p className="font-semibold text-sm text-slate-900">{mockDetail.joined_at}</p>
                </div>
                <div className="bg-[#F8F9FE] p-4 rounded-2xl border border-transparent hover:border-slate-100 transition-colors">
                  <p className="text-[11px] text-slate-500 mb-1 font-medium">Transaksi Terakhir</p>
                  <p className="font-semibold text-sm text-slate-900">{mockDetail.last_transaction_at}</p>
                </div>
                <div className="bg-green-50/80 p-4 rounded-2xl border border-transparent hover:border-green-100 transition-colors">
                  ...
                  <p className="font-semibold text-sm text-green-700">{formatCurrency(mockDetail.profit_income_from_agent)}</p>
                </div>
              </div>
            </div>
          </div>
