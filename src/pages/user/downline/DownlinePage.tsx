  ...
  if (isError || !dashboard) {
    return (
      <div className="flex-1 flex items-center justify-center min-h-[50vh] text-center p-4">
        <p className="text-destructive font-semibold">Gagal memuat daftar Downline.</p>
      </div>
    );
  }

  const pendingCount = dashboard.pending_invitations_received?.length || 0;

  const filteredAgents = dashboard.downliners.filter((agent) => {
    const matchSearch = agent.user_name.toLowerCase().includes(searchQuery.toLowerCase()) || agent.phone_number.includes(searchQuery);
    const matchStatus = filterStatus === "ALL" ? true : agent.user_status === filterStatus;
  ...
  return (
    <div className="flex-1 space-y-8 p-4 md:p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 md:gap-0">
        ...
        <div className="flex items-center gap-3">
          <Link to="/downline/incoming">
            <Button variant="outline" className="rounded-full px-6 font-medium text-primary border-primary hover:bg-primary/5 hover:text-primary relative">
      ...
      {filteredAgents.length === 0 ? (
        <div className="text-center py-10 text-muted-foreground">Tidak ada agent yang ditemukan.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {filteredAgents.map((agent) => (
            <Link to={`/downline/${agent.user_id}`} key={agent.user_id} className="block">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-9 w-9 border border-slate-100 bg-slate-50">
                        <AvatarFallback className="bg-primary/10 text-primary font-semibold">{agent.user_name.charAt(0).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div>
                        <h3 className="font-medium text-[15px] text-slate-800 leading-tight">{agent.user_name}</h3>
                        <p className="text-xs text-slate-500 mt-0.5">{agent.phone_number}</p>
                      </div>
                    </div>
